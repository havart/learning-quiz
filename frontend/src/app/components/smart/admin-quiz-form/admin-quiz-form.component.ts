...
export class AdminQuizFormComponent implements OnInit {
  ...
  questionsTitlesArray: Array<string> = [];
  chooseQuestionId: string;
  chooseQuestionTitle: string;
  questionsIdArray: Array<string> = [];
  ...
  private loadQuizById(quizId: string) {
    this.quizService.getByParam<IQuiz>(quizId)
      .subscribe(
        quiz => this.fillFormWithQuiz(quiz),
      );
  }
